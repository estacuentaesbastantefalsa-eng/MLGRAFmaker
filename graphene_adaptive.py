...
Fix, analogous to what the CNT chapter does analytically:
  1. For each kx (so omega = kx*v is fixed), find the real root(s) ky_res of
     the gamma=0 dispersion condition det(M_0(kx,ky)) = 0 -- this is exactly
     the plasmon resonance condition, "kv line meets dispersion curve".
  2. Integrate over ky with scipy.integrate.quad_vec, telling it where the
     resonance sits via the `points` argument so its adaptive refinement
     starts right there, and iterating until a requested error tolerance
     (epsabs/epsrel) is met -- so, unlike the fixed-grid version, you get an
     actual convergence guarantee (or an honest error estimate) instead of
     silently-wrong numbers.
  3. The outer kx integration is still done with a regular grid + trapezoid
     rule (that direction is smooth -- no narrow resonance in kx alone for
     fixed ky), reusing the same Cos/Sin-transform trick as before.
...
def wakefields_zeta_z_adaptive(g, zeta_nm, z_nm, kx_max=2.5, n_kx=200,
    ...
    for ikx, kx in enumerate(kx_grid):
        res_ky = [r for r in _find_ky_resonances(g, kx, ky_max, ky_scan_points) if 0 < r < ky_max]
        ...
        if res_ky:
            result, err = quad_vec(integrand, 0.0, ky_max, points=res_ky,
        ...
        if verbose and ikx % max(1, n_kx // 10) == 0:
            print(f"  kx[{ikx}/{n_kx}]={kx:.4f}  resonances found: {len(res_ky)}  quad err~{ky_errors[ikx]:.2e}")
